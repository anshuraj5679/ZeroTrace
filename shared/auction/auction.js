import { PRICE_SCALE, priceQuotePerBaseScaled, unscalePrice } from "./pricing.js";
export function runAuction(orders) {
    ...
    if (buys.length === 0 || sells.length === 0)
        return { clearingPriceQuotePerBase: 0n, clearingPriceQuotePerBaseScaled: 0n, matches: [] };
    if (buys[0].p < sells[0].p)
        return { clearingPriceQuotePerBase: 0n, clearingPriceQuotePerBaseScaled: 0n, matches: [] };
    let i = 0, j = 0;
    ...
    while (bi < i && si < j) {
        ...
        if (buyRemAsset === 0n || buyRemCash === 0n) {
            bi++;
            buyRemAsset = buys[bi]?.o.remainingRequest ?? 0n;
            buyRemCash = buys[bi]?.o.remainingDeposit ?? 0n;
        }
        if (sellRemAsset === 0n) {
            si++;
            sellRemAsset = sells[si]?.o.remainingDeposit ?? 0n;
        }
    }
